...
```bash cURL
curl -X POST https://api.billing.upag.io/v1/invoices/inv_abc123xyz/items \
...
```bash cURL
curl -X PUT https://api.billing.upag.io/v1/invoices/inv_abc123xyz/items/ii_abc123xyz \
  -H "Authorization: Bearer {token}" \
  -H "Content-Type: application/json" \
  -d '{
    "priceId": "price_def456ghi",
...
```bash cURL
curl -X DELETE https://api.billing.upag.io/v1/invoices/inv_abc123xyz/items/ii_abc123xyz \
  -H "Authorization: Bearer {token}"
```
